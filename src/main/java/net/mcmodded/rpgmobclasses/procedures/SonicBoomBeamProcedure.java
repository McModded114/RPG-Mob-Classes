...
public class SonicBoomBeamProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
		...
		if (!(entity instanceof Player _player ? _player.getCooldowns().isOnCooldown(itemstack.getItem()) : false)
				&& ((LivingEntity) entity).getAttribute(RpgMobClassesModAttributes.MANA.get()).getBaseValue() >= ((LivingEntity) entity).getAttribute(RpgMobClassesModAttributes.MAXMANA.get()).getBaseValue() * 0.5) {
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(itemstack.getItem(), 300);
			beam = new DamageSource(((Level) world).registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.SONIC_BOOM), entity) {
				@Override
				public Component getLocalizedDeathMessage(LivingEntity _livingEntity) {
					...
					if (_attacker != null && _itemName != null) {
						return Component.translatable("death.attack." + "sonic_boom.item", _entityName, _attackerName, _itemName);
					} else if (_attacker != null) {
						return Component.translatable("death.attack." + "sonic_boom.player", _entityName, _attackerName);
					} else {
						return Component.translatable("death.attack." + "sonic_boom", _entityName);
					}
				}
			};
			if (!(new Object() {
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof Player) {
						if (entityiterator instanceof Player _player && !_player.level.isClientSide())
							_player.displayClientMessage(Component.literal((entity.getDisplayName().getString() + ": " + "Sonic Boom!")), false);
					}
				}
			}
